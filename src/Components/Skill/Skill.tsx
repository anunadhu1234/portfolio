...
export function Skill() {
  ...
  return <div className="w-[100%] flex flex-col pt-12 items-center bg-[#0b071e]">
    ...
    <motion.div  className="flex flex-wrap justify-center mt-3 mb-1">
      {btn.map((item, index) => (
        <motion.button initial={{y:50, opacity:0}} whileInView={{y:0,opacity:1}} transition={{duration: 0.5, delay:index*0.5+1}} viewport={{once:false, amount:0.1}} key={index} onClick={()=>{setI(index); console.log(i);
if(item!=="All"){
  setTest(testimonials.filter((test) => test.designation === item));
    ...
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false, amount: 0.1 }} transition={{ duration: 1, delay:5 }}><AnimatedTestimonials testimonials={test} /></motion.div> 
    </div>;
}
